...
|--------|------|------------|
<<<<<<< Updated upstream
| `ExprKind` enum size reduction | `ExprKind` is dominated by three inline structs: `Heredoc { label: String, parts: ArenaVec }` (48 B), `Nowdoc { label: String, value: String }` (48 B), and `StaticMethodCall(StaticMethodCallExpr)` (56 B — contains a `Cow` + `ArenaVec`). This makes `ExprKind` ~64 B and `Expr { kind, span }` ~72 B. Every Pratt parser frame returns `Expr` by value, so this directly inflates stack usage and register pressure in the expression hot path. Arena-indirect these three variants (and `MethodCall` at 40 B) to bring `ExprKind` to ~32 B. | Medium |
| `Heredoc`/`Nowdoc` labels as `&'src str` | The heredoc label (e.g. `EOT`, `SQL`) is always present verbatim in the source but stored as `String`, causing a heap allocation per heredoc. Change to `&'src str`. `Nowdoc.value` can use `Cow<'src, str>` — `Borrowed` for non-indented nowdocs, `Owned` (arena string) for indented. This also reduces the size of the `Heredoc`/`Nowdoc` variants, feeding the `ExprKind` size reduction above. | Low |
| `interpolation.rs` zero-copy literal segments | `parse_interpolated_parts` opens with `String::new()` and accumulates every character via `push`, even for runs with no escape sequences. For escape-free segments, track the start of the run and emit `Cow::Borrowed(&inner[start..end])` directly. Only materialise an owned `String` when a `\n`/`\x..` escape is encountered. Most real strings have no escapes; this makes the common path allocation-free. | Low |
| `advance()` borrow workaround | `advance()` does `let lex_errs: Vec<_> = self.lexer.errors.drain(..).collect()` to avoid the borrow conflict between `drain` and `self.error()`. Replace with `std::mem::take(&mut self.lexer.errors)` — moves the existing `Vec` out (no allocation) and leaves an empty one behind. | Low |
| Pratt BP functions: by-value + `#[inline(always)]` | `infix_binding_power`, `prefix_binding_power`, and `postfix_binding_power` take `&TokenKind`. `TokenKind` is `Copy` and 4 bytes; the reference forces an indirect load and prevents enregistration. Change to pass by value and add `#[inline(always)]`. These are called for every token in the expression loop. | Low |
| `parse_simple_type` dead code and keyword-type allocation | The `Identifier` branch in `parse_simple_type` checks `eq_ignore_ascii_case("null")`, `"true"`, and `"false"` — impossible comparisons since the lexer already resolves those to `TokenKind::Null/True/False`. Remove them. Separately, keyword types (`self`, `parent`, `static`, `array`, etc.) construct a full `Name { parts: alloc_vec_one(...) }` when they could be dedicated `TypeHintKind` variants, eliminating the inner `ArenaVec` allocation for every typed parameter and return type. | Low |
| Arena pre-sizing | `bumpalo::Bump::new()` starts with a 512 B chunk and doubles on exhaustion. For a 100 KB PHP file with thousands of AST nodes this causes several chunk reallocations. The caller or the public `parse()` helper should use `Bump::with_capacity(source.len() * 3)` to pre-allocate one chunk large enough for the full parse. | Low |
=======
| `ExprKind` enum size reduction | `ExprKind` is dominated by three inline structs: `Heredoc { label: &'src str, parts: ArenaVec }` (40 B), `Nowdoc { label: &'src str, value: Cow }` (32 B), and `StaticMethodCall(StaticMethodCallExpr)` (56 B — contains a `Cow` + `ArenaVec`). This makes `ExprKind` ~64 B and `Expr { kind, span }` ~72 B. Every Pratt parser frame returns `Expr` by value, so this directly inflates stack usage and register pressure in the expression hot path. Arena-indirect `StaticMethodCall` and `MethodCall` to bring `ExprKind` to ~32 B. | Medium |
