...
| `parse_atom` variable fast-path | `$variable` is the most common atom in real PHP code but falls through a large `match` before being reached. Add an early-exit branch before the full match: `if self.current.kind == TokenKind::Variable { return self.parse_variable_expr(); }`. Saves a branch misprediction per expression in identifier-heavy code. | Very Low |
...
| Pre-sized `ArenaVec` capacity heuristics | Increase capacity hints for inner vecs based on context: class body → 16, function params → 4, array literals → 8, `use` imports → 4. Avoids bump-arena reallocation for typical PHP patterns. | Low |
